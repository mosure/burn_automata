...
The unit test `perceive_matches_upstream_sph_kernel_constants` pins the normalized kernel constants. `scripts/validate_import_parity.py` runs end-to-end CPU or WGPU rollout comparisons between Rust BPK inference and a dependency-free Python implementation of the same formulas, and reports deterministic 2D raster PSNR for imported 2D models.
...
The JSON interchange fallback remains useful for debugging and unsupported upstream variants:
...
```json
{
  ...
  "w2": [],
  "b2": [],
  "source": "optional checkpoint path"
}
```

The script in `scripts/export_npa_checkpoint.py` is intentionally conservative: it reads a PyTorch checkpoint through Python, finds two linear layers, writes row-major matrices, and leaves unsupported decoder heads for a later schema revision.
