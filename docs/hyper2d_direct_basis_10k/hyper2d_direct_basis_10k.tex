...
To calibrate adapter quality, selected train and holdout samples are compared against independently overfit 2D NPA oracle models trained from scratch on the same target objective. The oracle is not a hypernetwork and does not share weights across samples; it estimates the per-sample quality ceiling under the current NPA architecture and training recipe. The broader validation uses 8 train and 8 holdout oracle fits for 2048 epochs each, but these oracle fits also use the low-count 64-particle objective and are not a substitute for 2048-particle validation.
...
oracle examples per split & 8 & 8 & pass \\
direct-basis oracle max ratio & 1.20x & 1.141 train / 1.126 holdout & pass \\
\bottomrule
\end{tabular}
\caption{Quality gate audit for the 10k pilot. Despite acceptable low-count oracle ratios, the rollout and target-sample scale is below the current quality threshold, so the artifact status is \texttt{quality\_particle\_count\_too\_low}.}
\end{table}
...
The train and holdout curves are close, indicating that the shared basis is not simply memorizing train-only dynamics at this pilot scale. The oracle ratios measure how far the shared-base plus adapter representation remains from single-sample overfit NPAs under the same low-count objective. Ratios near one would indicate parity with a fully overfit oracle; larger ratios indicate remaining adapter/base expressivity or optimization gap. In this run, train and holdout oracle ratios are similar: 1.079 on train and 1.075 on holdout. This is evidence that the shared-basis direction is worth scaling, but it does not establish quality-ready shared-basis parity because the rollout and target-sampling scale are far below the 2048-particle / 2048-target-sample gate. It also does not prove image-conditioned generalization.

\section{Reproducibility Artifacts}
The companion Markdown report and JSON summary are audited by the Burn-first \path|report-hyper2d| CLI from the 10k training report and the 8x8 oracle-validation report. The generated rollout grid in Figure~\ref{fig:rollouts} selects high-ratio train and holdout cases from the low-count persisted oracle set, making the visualization intentionally stress the samples where the shared-basis representation is furthest from oracle parity.

\section{Limitations and Next Experiment}
This report does not validate generalized image-to-LoRA inference, and it should not be used as a paper-quality comparison against single-sample 2D oracles. The immediate correction is to rerun direct-basis training and oracle validation with at least 2048 rollout particles, at least 2048 target samples, and persisted oracle checkpoints. Only after that quality-scale adapter bank is validated should the next stage train a DINO-family image-condition encoder and rectified-flow LoRA generator, then evaluate generated adapters against direct adapters and oracle overfits on the same train/holdout samples and rollout stability panels.
