...
\graphicspath{{hyper_npa_figures/}}
\newcommand{\npa}{\mathrm{NPA}}
\newcommand{\hypernpa}{\mathrm{HyperNPA}}
\newcommand{\figroot}{latest}
...
shared base. A 16-target direct-adapter control reaches 23.58 dB aggregate but
only 19.30 dB p10, showing that recurrent substrate optimization remains a
large bottleneck before amortization. On two strictly held-out targets with
...
controllability, but not oracle-quality generalization.
\end{abstract}
...
\begin{table}[t]
\caption{Non-amortized 16-identity capacity control. These are training
identities, not held-out results. Throughput is median optimizer throughput.}
\label{tab:capacity}
\centering
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lrrrr}
...
frozen base, table refine & \textbf{23.58} & \textbf{19.30} & \textbf{15.46} & 4.03 \\
\bottomrule
\end{tabular}
}
\end{table}

Increased particle density and exposure produce large gains, proving that the
shared-base-plus-adapter substrate is substantially more expressive than the
short 1k run suggests. However, the 4,096-particle continuation selected its
initial checkpoint, indicating regression rather than benefit. Freezing the
base and refining only table rows raises aggregate PSNR by 2.88 dB, exposing
optimizer interference and shared-trunk drift. The remaining 19.30 dB p10 is
still far below the 26 dB gate and includes no unseen-image inference.
...
Third, the direct table improves dramatically with exposure and particle
density, reaching 23.58 dB aggregate. Conditional control, runtime
materialization, and GPU training are therefore alive.
...
\begin{enumerate}
  ...
  direct-adapter training with a frozen, parity-validated trunk and select on
  4,096-particle multi-horizon p10. The current 19.30 dB p10 is the immediate
  ceiling for amortization.
...
\begin{itemize}
  ...
  \item direct table: \path{artifacts/hyper2d_omnisvg_16_p4096_frozen_table_lr2e6/report.json}; and
