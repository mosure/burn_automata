...
The full ablation tables, BVH/tile/hash-grid candidate analysis, and paper-style discussion are generated by:
...
- `docs/kernel_ablations.tex`
- `docs/kernel_ablations.pdf`
...
The subgroup cooperative sweep used for the current opt-in decision is saved by:

```bash
scripts/bench_gpu_matrix.py --no-build --step-timing \
  --preset growing-2d --preset texture-2d --preset growing-3d-gs \
  --particles 4096 --particles 8192 --steps 8 --repeats 3 \
  --geometry dense --geometry point --geometry micro-cluster \
  --mode auto --mode cooperative-sorted-cells \
  --mode subgroup-cooperative-sorted-cells \
  --output target/bench_gpu_subgroup_cooperative_8k.json --timeout 180
```
...
```bash
scripts/bench_gpu_matrix.py --no-build --gaussian \
  --output target/bench_gpu_gaussian_write.json \
  --preset growing-3d-gs --preset texture-2d --preset growing-2d \
  --particles 4096 --particles 16384 --steps 4 --mode auto
```

The seed-scale sensitivity sweep is saved by:

```bash
scripts/bench_seed_scale_matrix.py --no-build \
  --output target/bench_seed_scale_matrix.json
scripts/bench_seed_scale_matrix.py --no-build --fixed-eps \
  --output target/bench_seed_scale_matrix_fixed_eps.json
```
