...
`configs/verified/2d/hypernpa/flow/smoke_conditional_row_flow.toml`. The canonical
teacher-free command smoke is
`configs/verified/2d/hypernpa/e2e/smoke_conditional_row_flow_e2e.toml`, and the
...
target/debug/bevy_automata export \
  --output-dir docs/papers/hypernpa/figures/heldout/<sample> \
...
`hyper_npa.tex`, `hyper_npa.bib`, and `figures/` are bundled
