...
LIZARD_BPK="${LIZARD_BPK:-/tmp/burn_automata_lizard.bpk}"
POLKA_BPK="${POLKA_BPK:-/tmp/burn_automata_polka.bpk}"
...
validate_bpk() {
  ...
  python3 scripts/validate_import_parity.py \
    --model "$model_path" \
    --particles "$PARTICLES" \
    --preset "$preset" \
...
if [[ "$CATALOG_PARITY" == "1" ]]; then
  python3 scripts/validate_catalog_parity.py \
