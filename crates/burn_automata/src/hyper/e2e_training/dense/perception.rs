        ...
        #[cfg(any(feature = "backend_wgpu", feature = "backend_cuda"))]
        {
            ...
            if let Some(device_adjoint) = device_adjoint {
                ...
                let device_adjoint = device_adjoint
                    .unwrap_or_else(|err| panic!("perception cube adjoint failed: {err}"));
    ...
    let (output, prepared) = {
        ...
        if let Some(device_forward) = prepared_forward {
            ...
            let prepared = PerceptionPreparedState {
                ...
                block_info: device_forward.block_info,
        ...
        } else if let Some(device_forward) =
            InnerBackend::perception_cube_forward(x_inner.clone(), s_inner.clone(), cube_config)
        {
            PERCEPTION_CUBE_FORWARD_DEVICE_HITS.fetch_add(1, Ordering::Relaxed);
            (
                device_forward
...
pub(super) fn perception_reference_options(
    _grid_eps: f32,
...
pub(super) fn dense_perception_batch(
    x: &Tensor3,
    s: &Tensor3,
    config: DirectBasisTrainConfig,
) -> Tensor3 {
...
pub(super) fn dense_perception_batch_chunk_generic<B: burn::tensor::backend::Backend>(
    ...
    let state_grad = state_diff
        ...
        .sum_dim(2)
        .squeeze_dim::<4>(2);
    let state_grad = apply_moment_correction_2d_batch_generic::<B>(state_grad, diff, volume_grad);
