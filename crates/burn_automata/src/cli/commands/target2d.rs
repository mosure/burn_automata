...
pub(crate) fn run_train_target_2d(command: Command) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let training = train_target_2d(&mut model, &hashgrid, &target, training_config, loss_config)?;
    ...
    let reference_loss = reference_model
        ...
        .transpose()?;
    let (gap, ratio) = loss_gap_and_ratio(training.final_loss.total_loss, reference_loss);
    ...
    write_json_report(&output, &report)?;
    println!(
        "wrote {} target_points={} final_loss={:.6} best_loss={:.6} reference_loss={} ratio={}",
        output.display(),
        report.target_points,
        report.training.final_loss.total_loss,
        report.training.best_loss.total_loss,
        report
            .reference_loss
            .map(|loss| format!("{:.6}", loss.total_loss))
            .unwrap_or_else(|| "none".to_string()),
