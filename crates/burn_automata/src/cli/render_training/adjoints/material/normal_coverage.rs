...
#[allow(clippy::too_many_arguments)]
pub(crate) fn material_surface_normal_opacity_updates(
    config: &NpaConfig,
    ...
    material_surface_normal_opacity_updates_weighted(
        config,
        target,
        positions,
        states,
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn material_surface_normal_opacity_updates_weighted(
    config: &NpaConfig,
    target: &TriangleMeshTarget,
    positions: &[[f32; 4]],
    states: &[f32],
    ...
    let candidate_rows = material_normal_candidate_rows(config, positions, states, row_weights);
    ...
    for sample_idx in 0..sample_count {
        ...
        let mut best_candidate = None::<(usize, f32)>;
        ...
        for &row in &candidate_rows {
            ...
            if best_candidate.is_none_or(|(_, best_distance2)| distance2 < best_distance2) {
                best_candidate = Some((row, distance2));
            }
        ...
        if let Some((row, distance2)) = best_candidate
    ...
    let target_bin_fraction = GROWTH_3D_MIN_SURFACE_NORMAL_BIN_FRACTION
        .max(GROWTH_3D_MIN_SURFACE_NORMAL_MEAN_BIN_COVERAGE)
        .clamp(0.0, 1.0);
...
pub(crate) fn add_material_surface_normal_state_adjoint(
    config: &NpaConfig,
    target: &TriangleMeshTarget,
    positions: &[[f32; 4]],
    states: &[f32],
...
fn material_normal_candidate_rows(
    config: &NpaConfig,
    positions: &[[f32; 4]],
    states: &[f32],
    row_weights: Option<&[f32]>,
