...
objective as native training with a 256-particle, batch-one, 8-16-step bounded
session profile so rendering and optimization can safely share a browser GPU.

GitHub-hosted Pages runners have no supported WebGPU adapter, so the deployment
gate uses `--static` to compile both Wasm modules, instantiate the training
worker without initializing its GPU backend, and verify every deployed model
URL before and after deployment.
